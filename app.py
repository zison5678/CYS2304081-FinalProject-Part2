...
MODEL_PATH = BASE_DIR / "model" / "lightweight_resolution_model.joblib"
MODEL_METADATA_PATH = BASE_DIR / "model" / "lightweight_model_metadata.json"
...
app = FastAPI(
    ...
    description=(
        ...
        "lightweight Random Forest model trained from the cleaned Part I data."
    ),
    version="2.0.0",
)
...
def predict(data: IncidentInput) -> PredictionOutput:
    try:
        ...
        predicted_hours = max(0.0, float(model_pipeline.predict(input_df)[0]))
